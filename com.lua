...
function com.def(objects_def)
	for i, object_def in ipairs(objects_def) do
		...
		local stored_version = {iid = win32.guid(object_def.iid)}
...
win32.ole32.CoInitialize(nil)

local CLSCTX_ALL = win32.ole32.CLSCTX_INPROC_SERVER + win32.ole32.CLSCTX_INPROC_HANDLER + win32.ole32.CLSCTX_LOCAL_SERVER
...
function com.new(clsid, name)
	...
	if (win32.ole32.CoCreateInstance(clsid, nil, win32.ole32.CLSCTX_INPROC_SERVER, com.iidof(name), ffi.cast('void**', temp)) == 0) then
		return ffi.gc(temp[0], com_gc)
	else
		return nil
	end
